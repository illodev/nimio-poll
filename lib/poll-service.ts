...
import { buildPollBlocks, buildHelpBlocks, buildPollListBlocks } from './blocks';
...
export async function createPoll(
  ...
  if (!payload.text || payload.text.trim().toLowerCase() === 'help') {
    return {
      success: true,
      message: JSON.stringify({ blocks: buildHelpBlocks(), response_type: 'ephemeral' }),
    };
  }

  // Check for list command
  if (payload.text.trim().toLowerCase() === 'list') {
    const polls = await Storage.getChannelPolls(payload.team_id, payload.channel_id);
  ...
  try {
    ...
    return { success: false, error: 'Error al crear la encuesta. Verifica los permisos del bot.' };
  }
}
  ...
  if (existingVoteIndex >= 0) {
    ...
    if (!poll.settings.multipleChoice) {
      ...
      const userVoteCount = poll.options.reduce(
        (count, opt) => count + opt.votes.filter(v => v.userId === userId).length,
        0
      );
